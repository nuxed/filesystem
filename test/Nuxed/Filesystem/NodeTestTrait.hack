...
trait NodeTestTrait {
  ...
  abstract public function provideNodes(
  ): Awaitable<Container<(Filesystem\Node)>>;
  abstract public function provideMissingNodes(
  ): Awaitable<Container<(Filesystem\Node)>>;

  public async function provideNodesPair(
  ): Awaitable<Container<(Filesystem\Node, Filesystem\Node)>> {
    $nodes = vec(await $this->provideNodes());
    $missing = vec(await $this->provideMissingNodes());
  ...
  public async function provideExistingNodesPair(
  ): Awaitable<Container<(Filesystem\Node, Filesystem\Node)>> {
    $a = vec(await $this->provideNodes());
    $b = Vec\reverse(await $this->provideNodes());
    $ret = vec[];
  ...
  public async function provideMissingNodesPair(
  ): Awaitable<Container<(Filesystem\Node, Filesystem\Node)>> {
    $a = vec(await $this->provideMissingNodes());
    $b = Vec\reverse(await $this->provideMissingNodes());
